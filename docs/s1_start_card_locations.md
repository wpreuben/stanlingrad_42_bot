...
추가된 `images_high/`의 파일명을 이미지 픽셀 내용으로 기존 불투명 파일과 대조했다. 지도 시작 카운터 171개 모두 고유한 이름의 시작 면과 연결됐고, 그중 138개는 같은 카운터의 다른 면도 연결됐다. [`s1_counter_named_images.csv`](s1_counter_named_images.csv)에 카드 위치·순서, 인쇄 표기, 카운터 파일명, 시작 면과 다른 면을 함께 기록했다. `s1_counter_image_checks.csv`의 19자리 `upload_id` 306개는 각각 제공된 원본 이미지 파일을 가리킨다. 이와 별도로 이름 붙은 파일에서 다른 면 3개를 확인했다. 이미지의 `-r` 면으로 시작하는 카운터는 24개다. `SO-261Inf-Div`의 기존 불투명 이미지 ID는 강한 `-f` 면이며, 시작 카드의 `1-3-3`은 이름 붙은 `-r` 면과 일치해 시작 면 기록을 수정했다.

이름 대조로 종전의 미확정 6개를 모두 확인했다. 추축군 `5`(`1936`)는 루마니아 제5기병사단, 소련군 `277`·`1 TD`(`1816`)는 각각 제277소총사단·제1전차구축사단, `5`(`2120`)는 제5기병군단의 감소 면, `275`(`2023`)는 제275소총사단, `4`(`2425`)는 제4소총사단 이미지다. 이름 대조 과정에서 서로 다른 카운터를 앞뒷면으로 연결한 7건을 수정하고, 시작 카드의 `45A`→`454`, `335+347`→`335+341` 판독을 바로잡았다. 다른 면 이미지가 없는 카운터의 단계 수는 여전히 추정하지 않는다.
