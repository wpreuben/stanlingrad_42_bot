...
추가된 `images_high/`의 파일명을 이미지 픽셀 내용으로 기존 불투명 파일과 대조했다. 지도 시작 카운터 171개 모두 고유한 이름의 시작 면과 연결됐고, 그중 135개는 같은 카운터의 다른 면도 연결됐다. [`s1_counter_named_images.csv`](s1_counter_named_images.csv)에 카드 위치·순서, 인쇄 표기, 카운터 파일명, 시작 면과 다른 면을 함께 기록했다. `s1_counter_image_checks.csv`의 19자리 `upload_id` 306개는 각각 제공된 원본 이미지 파일을 가리킨다. 이미지의 `-r` 면으로 시작하는 카운터 23개는 검수표에 감소된 면 시작으로 표시했다.
