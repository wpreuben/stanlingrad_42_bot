...
[`s1_start_card_fronts.csv`](s1_start_card_fronts.csv)에는 양측 지도 시작 카운터 171개(추축군 78개, 소련군 93개)의 **시작 카드에서 보이는 면**의 수치 문자열을 같은 순서로 옮겼다. 묶음별 카운터 수와 라벨 순서는 위치 CSV와 모두 일치한다. 카드는 일부 유닛을 이미 감소된 면으로 배치하므로 이 문자열을 무조건 카운터의 강한 면으로 해석할 수 없다. 탱크 점, 정예·낮은 질 표시, 검은 이동력 테두리, 단계 수, 국적·병과, 반대 면 수치는 아직 포함하지 않는다. `5[10]-5-4`는 카드에서 기본 공격 수치 `5` 위에 작은 `10`이 붙은 모습을 임시로 적은 것이며, 효과를 뜻하지 않는다. HQ의 `S(1)5`도 카드에 보이는 문자열로만 보관한다. 따라서 이 파일은 `UnitFace` 자료로 바로 변환할 수 없다.

[`s1_counter_image_checks.csv`](s1_counter_image_checks.csv)는 개별 85×85 카운터 이미지에서 시작 카드에 보이는 면과 다른 면을 확인한 부분 기록이다. 두 `upload_id`는 `Images/` 파일명에서 `ugc` 바로 뒤의 19자리 숫자이며, 해당 숫자를 포함하는 PNG 파일을 가리킨다. 같은 표기의 다른 군·병과 카운터가 있으므로 카드 위치·순서와 그림을 함께 대조했다. 소련군 `160`(`1502`)과 `62`(`1305`)는 시작 카드에 **감소된 면**이 보이며, 다른 면의 수치가 더 높다. 다른 면의 파일이 비어 있거나 수치가 없는 경우에는 그 카운터의 단계 수를 추정하지 않는다. 개별 이미지와 카드 수치가 다른 경우에는 규칙 판본과 카운터 교체 여부를 먼저 확인한다.
