...
카드에 보이는 면이 항상 강한 면은 아니다. `docs/s1_counter_image_checks.csv`와 `docs/s1_counter_named_images.csv`에 171개 시작 카운터의 이름·시작 면·확인된 다른 면을 기록했다. 개별 파일의 `-r` 면으로 시작하는 23개는 다른 면의 수치를 확인한 뒤 실제 시작 `steps`를 판정해 배치에 넣는다.

`images_high/`의 이름 붙은 이미지와 기존 `Images/`의 불투명 파일을 이미지 내용으로 대조해 시작 면 171개 전부를 확인했다. `454`(`1421`)와 `335+341`(`3322`) 표기도 원본을 확대해 수정했다. 같은 카운터의 다른 면이 확인되지 않는 경우 단계 수를 임의로 정하지 않는다.
